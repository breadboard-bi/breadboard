...
String sql = "";
String sql2 = "SELECT MAX(REQUISITION_DATE) AS REQUISITION_DATE FROM FACT_REQUISITION";
int myResolved = 0;
int mySLAcomp = 0;
...
if (category.length() > 0) {
   title = "Requisitions by " + temporal_value + " and " + business_unit + " and " + category;
   report_title = "Top Products by " + temporal_value + " and " + business_unit + " and " + category;
   dial_title = "Period Requisitions Trends for " + business_unit + " and " + category;
} else if ( business_unit.length() > 0)  {
   title = "Requisitions by " + temporal_value + " and " + business_unit;
   report_title = "Top Products by " + temporal_value + " and " + business_unit;
   dial_title = "Period Requisitions Trends for " + temporal_value + " and " + business_unit;
} else if (temporal_value.length() > 0)  {
   title = "Requisitions for " + temporal_value;
   report_title = "Requisitions for " + temporal_value;
   dial_title = "Period Requisitions Trends for " + temporal_value;
}
...
StringBuffer content_requisition_trend_line = new StringBuffer();
...
try {
   ...
   rs = ps.executeQuery();
   if (rs.next()) {
      REQUISITION_DATE = "Updated with requisition data as of "+rs.getString("REQUISITION_DATE")+" PST";
   }
   if (rs != null) rs.close(); rs = null;
   if (ps != null) ps.close(); ps = null;
...
if (temporal_cat == "Year of " || temporal_cat == "Quarter of ") {
   ...
   parameters.setParameter( "drill-url", "requisition_interactive_line_dashboard.jsp?temporal_value={SERIES}" );
}
...
ChartHelper.doChart("breadboard", "supply_chain/requisitions/dashboards",
                    "requisition_period_vert_bar.widget.xml", parameters,
...
//define the click url template
parameters.setParameter("drill-url", "requisition_interactive_line_dashboard.jsp?temporal_value="
...
ChartHelper.doChart("breadboard", "supply_chain/requisitions/dashboards",
                    "requisition_period_bu_vert_bar.widget.xml", parameters,
                    content_bu_vert_bar, userSession, messages, null );



// Make a bar chart showing the product category, default business_unit Store9
// create the parameters for the bar chart
parameters = new SimpleParameterProvider();
// define the click url template
parameters.setParameter( "drill-url", "requisition_interactive_line_dashboard.jsp?temporal_value="+temporal_value
...
//use the chart definition in 'samples/dashboard/regions.widget.xml'
ChartHelper.doChart("breadboard", "supply_chain/requisitions/dashboards",
                    "requisition_period_bu_pcat_vert_bar.widget.xml", parameters,
...
SolutionHelper.doAction("breadboard", "supply_chain/requisitions/dashboards/reports",
                     "requisition_top_products_report.xaction", "requisition_interactive_line_dashboard.jsp",
...
// use the chart definition in 'samples/dashboard/regions.widget.xml'
ChartHelper.doChart( "breadboard", "supply_chain/requisitions/dashboards", 
					  "requisition_period_bu_pcat_line.widget.xml", parameters, 
					  content_requisition_trend_line, userSession, messages, null ); 
...
<html>
<head>
<title>Breadboard BI Sample Requisition Dashboard</title>
...
<form name="form1" action="requisition_interactive_line_dashboard.jsp" method="GET">
...
<!--			Click on an Requisition ID to drill to another page (not activated) -->
<BR>
...
<!-- Top of #6 -->
<%= content_requisition_trend_line.toString() %>
