...
<html>
	<head>
		<title>Breadboard BI Sample Case Management Dashboard</title>
	</head>
	<body>
...
<%
	...
	if( type != null ) {
		title = "Cases by " + year_quarter + " and " + category + " and " + type;
	} 
...
<!-- include file="navBarTopSimple.html"  -->
 ...
 <font style='font-family:Arial'><B>Interdependent Widget Sample Dashboard</B><BR>  
 <I>Gateway sample dashboard examples are also available.</I></font></td>
 </tr> 
...
<%
		...
		// define the click url template
	         parameters.setParameter( "drill-url", "cm_interactive_dashboard_I.jsp?year_quarter={SERIES}" );
...
<!-- Top #2 -->
	...
	<%
		...
		// define the click url template
	        parameters.setParameter( "drill-url", "cm_interactive_dashboard_I.jsp?year_quarter="+year_quarter+"&amp;category={SERIES}" );
		// define the slices of the pie chart
...
<!-- Top 3 -->	
	...
	<%
			...
			// create the parameres for the bar chart
	        	parameters = new SimpleParameterProvider();
			// define the click url template
	        parameters.setParameter( "drill-url", "cm_interactive_dashboard_I.jsp?year_quarter="+year_quarter+"&amp;category="+category+"&amp;type={SERIES}" );
			...
			// call the chart helper to generate the pie chart image and to get the HTML content
			// use the chart definition in 'samples/dashboard/regions.widget.xml'
	        	ChartHelper.doChart( "breadboard", "customer_360/case_management/dashboards", "case_type_vert_bar.widget.xml", parameters, content, userSession, messages, null ); 
	%>
			</span>
...
String sql = "SELECT ROUND(AVG(DATEDIFF(SYSDATE(),F.CASE_OPEN_DATE))) AS punta_id, round(sum(F.sla_response_compliance_qty*100)/count(F.case_sk),0) as sla_compliance_pct,round(sum(F.case_resolved_qty*100)/count(F.case_sk),0) as resolved_pct FROM FACT_CASE F, DIMENSION_CASE C, DIMENSION_DAY D WHERE F.CASE_SK = C.CASE_SK AND F.CASE_OPEN_DATE_SK = D.DATE_SK AND D.YEAR_QUARTER_NAME = ? AND C.CASE_SOURCE_NAME = ? AND C.CASE_TYPE_NAME = ?";
...
ps.setString(3, myType);
   ...
   if (rs.next()) {
	...
	mySLAcomp = rs.getInt("sla_compliance_pct");
   }
...
%>
	<tr>
		<td valign="top" style="font-family:Arial;font-weight:bold;width: 33%">
<!-- Top of #4 -->
	<%
		if( year_quarter != null ) {
...
<!-- Top of #6 -->
	<%
		if( year_quarter != null ) {
			...
			int dialValue = mySLAcomp;
		    messages = new ArrayList();
        	parameters = new SimpleParameterProvider();
        	parameters.setParameter( "image-width", "105"); //$NON-NLS-1$ //$NON-NLS-2$
        	parameters.setParameter( "image-height", "105"); //$NON-NLS-1$ //$NON-NLS-2$
			parameters.setParameter( "value", ""+dialValue );
			parameters.setParameter( "title", "Percent Cases SLA Compliant" );
			content = new StringBuffer(); 
        	ChartHelper.doDial( "breadboard", "customer_360/case_management/dashboards", "sample_dial_pct.widget.xml", parameters, content, userSession, messages, null ); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
%>
<%= content.toString() %>
