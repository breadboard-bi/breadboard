...
String sql = "";
String sql2 = "SELECT MAX(WAREHOUSE_RECEIPT_DATE) AS WAREHOUSE_RECEIPT_DATE FROM FACT_WAREHOUSE_RECEIPT";
int myResolved = 0;
int mySLAcomp = 0;
String exceptionErrors = "";
String title = "Sample Dashboard - Select a Business Unit";
String report_title = "";
String line_chart_title = "";
...
if (business_unit.length() < 1)
   business_unit = "Store9";
if (warehouse.length() < 1)
   warehouse = "Warehouse2";
if (category.length() < 1)
   category = "Drink";
// if (category.length() > 0) {
   title = "Warehouse Receipts by " + temporal_value + " & " + business_unit + " & " + category;
   report_title = "Top Products by " + temporal_value + " & " + business_unit + " & " + category + " & " + warehouse;
   line_chart_title = "Period Trends for " + business_unit + " & " + category + " & " + warehouse;
// } else if ( business_unit.length() > 0)  {
//   title = "Warehouse Receipts by " + temporal_value + " & " + business_unit;
//   report_title = "Top Products by " + temporal_value + " & " + business_unit;
//   line_chart_title = "Period Trends for " + temporal_value + " & " + business_unit;
// } else if (temporal_value.length() > 0)  {
//   title = "Warehouse Receipts for " + temporal_value;
//   report_title = "Warehouse Receipts for " + temporal_value;
//   line_chart_title = "Period Trends for " + temporal_value;
// }
String WAREHOUSE_RECEIPT_DATE = "";
StringBuffer content_region = new StringBuffer();
StringBuffer content_bu_vert_bar = new StringBuffer();
StringBuffer content_bu_pcat_vert_bar = new StringBuffer();
StringBuffer content_bu_pcat_ware_vert_bar = new StringBuffer();
StringBuffer content_sn_trend_line = new StringBuffer();
ByteArrayOutputStream stream_embedded_report = new ByteArrayOutputStream();
...
try {
   ...
   rs = ps.executeQuery();
   if (rs.next()) {
      WAREHOUSE_RECEIPT_DATE = "Updated with warehouse receipt data as of "+rs.getString("WAREHOUSE_RECEIPT_DATE")+" PST";
   }
   if (rs != null) rs.close(); rs = null;
   if (ps != null) ps.close(); ps = null;
...
if (temporal_cat == "Year of " || temporal_cat == "Quarter of ") {
   ...
   parameters.setParameter( "drill-url", "wr_interactive_line_dashboard.jsp?temporal_value={SERIES}" );
}
...
ChartHelper.doChart("breadboard", "supply_chain/warehouse_receipts/dashboards",
                    "wr_period_vert_bar.widget.xml", parameters,
...
//define the click url template
parameters.setParameter("drill-url", "wr_interactive_line_dashboard.jsp?temporal_value="
...
ChartHelper.doChart("breadboard", "supply_chain/warehouse_receipts/dashboards",
                    "wr_period_bu_vert_bar.widget.xml", parameters,
...
parameters = new SimpleParameterProvider();
// define the click url template
parameters.setParameter( "drill-url", "wr_interactive_line_dashboard.jsp?temporal_value="+temporal_value
                         +"&amp;business_unit="+business_unit+"&amp;category={SERIES}" );
parameters.setParameter( "BUSINESS_UNIT", business_unit);
parameters.setParameter( "TEMPORAL_VALUE", temporal_value);
...
//use the chart definition in 'samples/dashboard/regions.widget.xml'
ChartHelper.doChart("breadboard", "supply_chain/warehouse_receipts/dashboards",
                    "wr_period_bu_pcat_vert_bar.widget.xml", parameters,
                    content_bu_pcat_vert_bar, userSession, messages, null );
...
// define the click url template
parameters.setParameter( "drill-url", "wr_interactive_line_dashboard.jsp?temporal_value="+temporal_value
                         +"&amp;business_unit="+business_unit+"&amp;category="+category+"&amp;warehouse={SERIES}" );
parameters.setParameter( "BUSINESS_UNIT", business_unit);
parameters.setParameter( "TEMPORAL_VALUE", temporal_value);
parameters.setParameter( "CATEGORY", category );
parameters.setParameter( "outer-params", "WAREHOUSE" );
// define the business_unit axis of the bar chart
parameters.setParameter( "inner-param", "WAREHOUSE"); //$NON-NLS-1$ //$NON-NLS-2$
// set the width and the height
parameters.setParameter( "image-width", "400"); //$NON-NLS-1$ //$NON-NLS-2$
parameters.setParameter( "image-height", "300"); //$NON-NLS-1$ //$NON-NLS-2$
 messages = new ArrayList();
...
//use the chart definition in 'samples/dashboard/regions.widget.xml'
ChartHelper.doChart("breadboard", "supply_chain/warehouse_receipts/dashboards",
                    "wr_period_bu_pcat_ware_vert_bar.widget.xml", parameters,
                    content_bu_pcat_ware_vert_bar, userSession, messages, null );





// if the user has clicked on a bar of the bar chart we should have a year quarter, business_unit and category
...
parameters.setParameter( "BUSINESS_UNIT", business_unit );
parameters.setParameter( "CATEGORY", category );
parameters.setParameter( "WAREHOUSE", warehouse );
...
SolutionHelper.doAction("breadboard", "supply_chain/warehouse_receipts/dashboards/reports",
                     "wr_top_products_report.xaction", "wr_interactive_line_dashboard.jsp",
                     parameters, stream_embedded_report, userSession, messages, null );
...
// use the chart definition in 'samples/dashboard/regions.widget.xml'
ChartHelper.doChart( "breadboard", "supply_chain/warehouse_receipts/dashboards", 
					  "wr_period_bu_pcat_ware_line.widget.xml", parameters, 
					  content_sn_trend_line, userSession, messages, null ); 
...
<html>
<head>
<title>Breadboard BI Sample Warehouse Receipt Dashboard</title>
...
<form name="form1" action="wr_interactive_line_dashboard.jsp" method="GET">
...
<%= content_bu_pcat_vert_bar.toString() %>
<!-- Bottom of #3 -->
</td>
</tr>
...
<td valign="top" align="left">
                <span style="font-family:Arial;font-weight:bold">Select a Destination Warehouse</span>
<%= content_bu_pcat_ware_vert_bar.toString() %>
</td>
<!-- BOTTOM 4 -->
<!-- Top of #5 -->
<td valign="top" align = "center">
<BR><BR>
<font style="font-family:Arial;font-size:11pt;font-weight:bold">
<%= report_title %>
</font>
<%= stream_embedded_report.toString() %>
</td>
<td valign="top" align = "left">
<BR>
<%= content_sn_trend_line.toString() %>
</td>
<!-- Top of #6 -->
