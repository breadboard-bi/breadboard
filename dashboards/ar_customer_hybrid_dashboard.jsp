...
// Drop-down Variables
String selObj_customer = "";
String customer_sk = ((request.getParameter("customer_sk")!=null)?request.getParameter("customer_sk"):"");
...
String sql = "";
String sql2 = "SELECT MAX(DW_LOAD_DATE) AS DW_LOAD_DATE FROM FACT_ACCOUNT_RECEIVABLE";
String sql3 = "";
String sql_default_customer_sk = "SELECT CAST(MAX(F.CUSTOMER_SK)AS CHAR) AS DEFAULT_CUSTOMER_SK FROM FACT_ACCOUNT_RECEIVABLE F"; 
String sql_customer = "SELECT DISTINCT D.CUSTOMER_SK AS customer_table_id, D.CUSTOMER_NAME AS customer_name FROM DIMENSION_CUSTOMER D, FACT_ACCOUNT_RECEIVABLE F WHERE F.CUSTOMER_SK = D.CUSTOMER_SK ORDER BY D.CUSTOMER_NAME";
ArrayList customer_ids = new ArrayList();
ArrayList customer_names = new ArrayList();

String sql_customer_name = "SELECT DISTINCT CUSTOMER_NAME AS CUSTOMER FROM DIMENSION_CUSTOMER WHERE CUSTOMER_SK = ? ";                     
...
StringBuffer content_dial = new StringBuffer();
...
if (temporal_value.indexOf("W") >= 0) {
   ...
   sql  = "SELECT "+
          ...
          "WHERE "+
          "WEEK_NAME = ?";
   sql3 = "select ROUND(AVG(F.TRXN_RECEIVABLE_AMT)) as AVG_TRXN_AMT FROM FACT_ACCOUNT_RECEIVABLE F, DIMENSION_ACCOUNT A, DIMENSION_DAY D, DIMENSION_BUSINESS_UNIT B WHERE D.WEEK_NAME = ? AND F.CUSTOMER_SK = ? AND F.ACCOUNT_SK = A.ACCOUNT_SK AND F.JOURNAL_DATE_SK = D.DATE_SK AND F.BUSINESS_UNIT_SK = B.BUSINESS_UNIT_SK";
   ...
   sql  = "SELECT "+
          ...
          "WHERE "+
          "YEAR_QUARTER_NAME = ?";
   sql3 = "select ROUND(AVG(F.TRXN_RECEIVABLE_AMT)) as AVG_TRXN_AMT FROM FACT_ACCOUNT_RECEIVABLE F, DIMENSION_ACCOUNT A, DIMENSION_DAY D, DIMENSION_BUSINESS_UNIT B WHERE D.YEAR_QUARTER_NAME = ? AND F.CUSTOMER_SK = ? AND F.ACCOUNT_SK = A.ACCOUNT_SK AND F.JOURNAL_DATE_SK = D.DATE_SK AND F.BUSINESS_UNIT_SK = B.BUSINESS_UNIT_SK";
   ...
   sql3 = "select ROUND(AVG(F.TRXN_RECEIVABLE_AMT)) as AVG_TRXN_AMT FROM FACT_ACCOUNT_RECEIVABLE F, DIMENSION_ACCOUNT A, DIMENSION_DAY D, DIMENSION_BUSINESS_UNIT B WHERE D.MONTH_SK = ? AND F.CUSTOMER_SK = ? AND F.ACCOUNT_SK = A.ACCOUNT_SK AND F.JOURNAL_DATE_SK = D.DATE_SK AND F.BUSINESS_UNIT_SK = B.BUSINESS_UNIT_SK";
   ...
   sql  = "SELECT "+
          ...
          "WHERE "+
          "YEAR_SK = ?";
   sql3 = "select ROUND(AVG(F.TRXN_RECEIVABLE_AMT)) as AVG_TRXN_AMT FROM FACT_ACCOUNT_RECEIVABLE F, DIMENSION_ACCOUNT A, DIMENSION_DAY D, DIMENSION_BUSINESS_UNIT B WHERE D.YEAR_SK = ? AND F.CUSTOMER_SK = ? AND F.ACCOUNT_SK = A.ACCOUNT_SK AND F.JOURNAL_DATE_SK = D.DATE_SK AND F.BUSINESS_UNIT_SK = B.BUSINESS_UNIT_SK";
   ...
   sql3 = "select ROUND(AVG(F.TRXN_RECEIVABLE_AMT)) as AVG_TRXN_AMT FROM FACT_ACCOUNT_RECEIVABLE F, DIMENSION_ACCOUNT A, DIMENSION_DAY D, DIMENSION_BUSINESS_UNIT B WHERE D.DATE_SK = ? AND F.CUSTOMER_SK = ? AND F.ACCOUNT_SK = A.ACCOUNT_SK AND F.JOURNAL_DATE_SK = D.DATE_SK AND F.BUSINESS_UNIT_SK = B.BUSINESS_UNIT_SK";
} else {
   temporal_cat = "";
}
...
try {
   ...
   if (debugMode) msgs += "  START-->Query Default Customer SK= " +getTimeStamp()+ "<br>";
   //now get the next query results, String sql_customer
   ps = conn.prepareStatement(sql_default_customer_sk);
   rs = ps.executeQuery();
   if (rs.next()) {
      default_customer_sk = rs.getString("DEFAULT_CUSTOMER_SK");
   }
   if (rs != null) rs.close(); rs = null;
   if (ps != null) ps.close(); ps = null;
   if( customer_sk.length()<1) customer_sk = default_customer_sk; 


   if (debugMode) msgs += "  START-->Query Customer= " +getTimeStamp()+ "<br>";
   //now get the next query results, String sql_customer
   ps = conn.prepareStatement(sql_customer);
   rs = ps.executeQuery();
   while (rs.next()) {
    String id = ((rs.getString("customer_table_id")!=null)?rs.getString("customer_table_id"):"");
   ...
   if (ps != null) ps.close(); ps = null;
    ...
    if (debugMode) msgs += "  START-->Query Customer Name = " +getTimeStamp()+ "<br>";
   //now get the next query results, String sql_customer_name
   ps = conn.prepareStatement(sql_customer_name);
   ps.setString(1, customer_sk);
   rs = ps.executeQuery();
   if (rs.next()) {
   ...
   ps.setString(1, temporal_value);
   ps.setString(2, customer_sk);
   rs = ps.executeQuery();
   if (rs.next()) {
    	avg_base_amt = rs.getInt("AVG_TRXN_AMT");
   } 
   if (rs != null) rs.close(); rs = null;
   if (ps != null) ps.close(); ps = null;
...
dashboard_title = "Accounts Receivable Dashboard for Customer "+customer;
widget_1_title = "By the " + temporal_cat+" "+temporal_value;
widget_2_title = "Account Type by the " + temporal_cat+" "+temporal_value;
widget_3_title = "Business Unit by the " + temporal_cat+" "+temporal_value;


trend_line_amt_title = "Receivable Amounts for the " + temporal_cat+" "+temporal_value;
trend_line_qty_title = "Receivable Quantities for the " + temporal_cat+" "+temporal_value; 
...
parameters.setParameter( "drill-url", "ar_customer_hybrid_dashboard.jsp?temporal_value={SERIES}" );
}
parameters.setParameter( "CUSTOMER_SK", customer_sk );
...
ChartHelper.doChart( "breadboard", "finance/account_receivable/dashboards", 
					"ar_customer_period_vert_bar.widget.xml", parameters, 
...
// define the click url template
parameters.setParameter( "drill-url", "Pivot?solution=breadboard&path=finance/account_receivable/analysis&action=ar_customer_cube.xaction" );
// define the bars of the bar chart
parameters.setParameter( "CUSTOMER_SK", customer_sk );
parameters.setParameter( "TEMPORAL_VALUE", temporal_value);
...
// call the chart helper to generate the bar chart image and to get the HTML content
ChartHelper.doChart( "breadboard", "finance/account_receivable/dashboards",
 					"ar_customer_period_account_type_vert_bar.widget.xml", parameters, 
...
// call the chart helper to generate the bar chart image and to get the HTML content
ChartHelper.doChart( "breadboard", "finance/account_receivable/dashboards", 
					"ar_customer_period_bu_type_vert_bar.widget.xml", parameters, 
...
ChartHelper.doChart( "breadboard", "finance/account_receivable/dashboards", 
				"ar_customer_period_line.widget.xml", parameters, 
...
ChartHelper.doChart( "breadboard", "finance/account_receivable/dashboards",
 			"ar_customer_period_line_II.widget.xml", parameters, 
...
parameters.setParameter( "value", ""+avg_base_amt );
parameters.setParameter( "title", "Average Amount");
ChartHelper.doDial( "breadboard", "finance/account_receivable/dashboards", 
			"ar_customer_dial.widget.xml", parameters, 
			content_dial, userSession, messages, null ); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$


if (debugMode) msgs += "  END-->Charts = " +getTimeStamp()+ "<br><br><br>Sql1 = " 
                       +sql+ "<br><br>Sql2 = " +sql2+ "<br><br>Sql3 = " +sql3+ "<br><br>"+
                       "time="+temporal_value+"<br>";
...
for (int i =0; i <  customer_ids.size(); i++) {
	String customer_name = (String)customer_names.get(i);
	String customer_id = (String)customer_ids.get(i);
	selObj_customer += "<option value=\"" +customer_id+ "\">" +customer_name+ "</option>\n";
}
...
<html>
<head>
<title>Breadboard BI Sample Accounts Receivable Customer Dashboard</title>
...
function doDateChange() {
  ...
  function submitNewoption_customer(selObj_customer) {
    document.form1.submit();
 }
</script>
</head>
...
<form name="form1" action="ar_customer_hybrid_dashboard.jsp" method="GET">

<select name="customer_sk" onChange="submitNewoption_customer(this)">
<option value="<%= customer_sk %>">Select a Customer</option>
<%= selObj_customer %>
</select>
...
Note:  test data available for 2006.
</form>
...
<!-- Top of 1 -->
<%= widget_1_title %><br>
<small>click on a bar to change the time period</small>
...
<!-- Top of #4 -->
<BR>
<a href="Pivot?solution=breadboard&path=finance/account_receivable/analysis&action=ar_account_time_cube.xaction" target="_blank">
<small>click to open an Analysis cube in a new window</small>
