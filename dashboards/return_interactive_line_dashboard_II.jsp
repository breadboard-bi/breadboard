...
String return_type = ((request.getParameter("return_type")!=null)?request.getParameter("return_type"):"");
...
String sql = "";
String sql2 = "SELECT MAX(RETURN_DATE) AS RETURN_DATE FROM FACT_RETURN";
// Oracle - String sql2 = "SELECT TO_CHAR(MAX(RETURN_DATE),'MM-DD-YYYY HH24:MI:SS') AS RETURN_DATE FROM FACT_RETURN";
int myResolved = 0;
int mySLAcomp = 0;
String exceptionErrors = "";
String title = "Sample Dashboard - Select a reason";
String report_title = "";
String dial_title = "";
...
if (reason.length() < 1)
   reason = "Not Like Product";
if (return_type.length() < 1)
   return_type = "Standard Return";

String return_type_temp = return_type.replace(' ','+');
String reason_temp = reason.replace(' ','+');

if (return_type.length() > 0) {
   title = "Returns by " + temporal_value + " and " + reason + " and " + return_type;
   report_title = "Top Returned Products by " + temporal_value + " and " + reason + " and " + return_type;
   dial_title = "Period Return Trends for " + reason + " and " + return_type;
} else if ( reason.length() > 0)  {
   title = "Returns by " + temporal_value + " and " + reason;
   report_title = "Top Products by " + temporal_value + " and " + reason;
   dial_title = "Period Return Trends for " + temporal_value + " and " + reason;
} else if (temporal_value.length() > 0)  {
   title = "Returns for " + temporal_value;
   report_title = "Returns for " + temporal_value;
   dial_title = "Period Return Trends for " + temporal_value;
}
String return_date = "";
StringBuffer content_region = new StringBuffer();
StringBuffer content_reason_vert_bar = new StringBuffer();
StringBuffer content_reason_pcat_vert_bar = new StringBuffer();
...
try {
   ...
   rs = ps.executeQuery();
   if (rs.next()) {
      return_date = "Updated with return data as of "+rs.getString("RETURN_DATE")+" PST";
   }
   if (rs != null) rs.close(); rs = null;
   if (ps != null) ps.close(); ps = null;
...
if (temporal_cat == "Year of " || temporal_cat == "Quarter of ") {
   ...
   parameters.setParameter( "drill-url", "return_interactive_line_dashboard_II.jsp?temporal_value={SERIES}" );
}
...
ChartHelper.doChart("breadboard", "customer_360/returns/dashboards",
                    "return_period_vert_bar.widget.xml", parameters,
                    content_region, userSession, messages, null);



//Make a bar chart showing the return reason
//create the parameters for the bar chart
parameters = new SimpleParameterProvider();
//define the click url template
parameters.setParameter("drill-url", "return_interactive_line_dashboard_II.jsp?temporal_value="+temporal_value+"&amp;return_type="+return_type_temp+"&amp;reason={SERIES}" );
parameters.setParameter("REASON", reason_temp);
parameters.setParameter("RETURN_TYPE", return_type_temp);
parameters.setParameter( "TEMPORAL_VALUE", temporal_value);
//define the reason axis of the bar chart
parameters.setParameter( "inner-param", "REASON"); //$NON-NLS-1$ //$NON-NLS-2$
//set the width and the height
parameters.setParameter( "image-width", "400"); //$NON-NLS-1$ //$NON-NLS-2$
parameters.setParameter( "image-height", "300"); //$NON-NLS-1$ //$NON-NLS-2$
...
ChartHelper.doChart("breadboard", "customer_360/returns/dashboards",
                    "return_period_reason_vert_bar.widget.xml", parameters,
                    content_reason_vert_bar, userSession, messages, null );



// Make a bar chart showing the return type
// create the parameters for the bar chart
parameters = new SimpleParameterProvider();
// define the click url template
parameters.setParameter( "drill-url", "return_interactive_line_dashboard_II.jsp?temporal_value="+temporal_value+"&amp;reason="+reason_temp+"&amp;return_type={SERIES}" );
parameters.setParameter( "REASON", reason );
parameters.setParameter( "RETURN_TYPE", return_type );
parameters.setParameter( "TEMPORAL_VALUE", temporal_value);
parameters.setParameter( "outer-params", "RETURN_TYPE" );
// define the reason axis of the bar chart
parameters.setParameter( "inner-param", "RETURN_TYPE"); //$NON-NLS-1$ //$NON-NLS-2$
// set the width and the height
parameters.setParameter( "image-width", "400"); //$NON-NLS-1$ //$NON-NLS-2$
parameters.setParameter( "image-height", "300"); //$NON-NLS-1$ //$NON-NLS-2$
 messages = new ArrayList();
...
//use the chart definition in 'samples/dashboard/regions.widget.xml'
ChartHelper.doChart("breadboard", "customer_360/returns/dashboards",
                    "return_period_type_vert_bar.widget.xml", parameters,
                    content_reason_pcat_vert_bar, userSession, messages, null );




// if the user has clicked on a bar of the bar chart we should have a year quarter, reason and return_type
// run a report and embed the content into this page
// create the parameters for the report
parameters = new SimpleParameterProvider();
// pass the return_type, type, status parameters to the report
parameters.setParameter( "TEMPORAL_VALUE", temporal_value );
parameters.setParameter( "REASON", reason );
parameters.setParameter( "RETURN_TYPE", return_type );
...
SolutionHelper.doAction("breadboard", "customer_360/returns/dashboards/reports",
                     "return_top_products_report_II.xaction", "return_interactive_line_dashboard_II.jsp",
...
// define the return_type axis of the bar chart
parameters.setParameter( "inner-param", "RETURN_TYPE"); //$NON-NLS-1$ //$NON-NLS-2$
// set the width and the height
parameters.setParameter( "image-width", "400"); //$NON-NLS-1$ //$NON-NLS-2$
...
// use the chart definition in 'samples/dashboard/regions.widget.xml'
ChartHelper.doChart( "breadboard", "customer_360/returns/dashboards", 
					  "return_period_reason_type_line.widget.xml", parameters, 
...
<html>
<head>
<title>Breadboard BI Sample Sales Return Capture Dashboard</title>
...
<form name="form1" action="return_interactive_line_dashboard_II.jsp" method="GET">
...
<!-- Top #2 -->
                <span style="font-family:Arial;font-weight:bold">Select a Reason</span>
                <%= content_reason_vert_bar.toString() %>

<!-- BOTTOM 2 -->
        </td>
        <td valign="top" style="width: 33%"><span style="font-family:Arial;font-weight:bold">
<!-- Top 3 -->         Select a Return Type
                        </span>
                        <%= content_reason_pcat_vert_bar.toString() %>
<!-- Bottom of #3 -->
        </td>
        </tr>
        <tr>
