...
String sql1 = "SELECT "+
          ...
          "FROM "+
          "FACT_PROCUREMENT F"; 
String sql2 = "SELECT MAX(DW_LOAD_DATE) AS DW_LOAD_DATE FROM FACT_PROCUREMENT";
String sql3 = "SELECT "+
...
String sql_prod = "SELECT DISTINCT D.PRODUCT_SK AS table_id, D.PRODUCT_NAME AS name FROM DIMENSION_PRODUCT D, FACT_PROCUREMENT F WHERE F.PRODUCT_SK = D.PRODUCT_SK ORDER BY D.PRODUCT_NAME";
ArrayList prod_ids = new ArrayList();
ArrayList prod_names = new ArrayList();

String sql_bu = "SELECT DISTINCT D.BUSINESS_UNIT_SK AS bu_table_id, D.BUSINESS_UNIT_NAME AS bu_name FROM DIMENSION_BUSINESS_UNIT D, FACT_PROCUREMENT F WHERE F.BUSINESS_UNIT_SK = D.BUSINESS_UNIT_SK AND F.PRODUCT_SK = ? ORDER BY D.BUSINESS_UNIT_NAME";
ArrayList bu_ids = new ArrayList();
ArrayList bu_names = new ArrayList();

String sql_supplier = "SELECT DISTINCT D.SUPPLIER_SK AS supplier_table_id, D.SUPPLIER_NAME AS supplier_name FROM DIMENSION_SUPPLIER D, FACT_PROCUREMENT F WHERE F.SUPPLIER_SK = D.SUPPLIER_SK AND F.PRODUCT_SK = ? ORDER BY D.SUPPLIER_NAME";
ArrayList supplier_ids = new ArrayList();
ArrayList supplier_names = new ArrayList();

String sql_warehouse = "SELECT DISTINCT D.WAREHOUSE_SK AS warehouse_table_id, D.WAREHOUSE_NAME AS warehouse_name FROM DIMENSION_WAREHOUSE D, FACT_PROCUREMENT F WHERE F.WAREHOUSE_SK = D.WAREHOUSE_SK AND F.PRODUCT_SK = ? ORDER BY D.WAREHOUSE_NAME";
...
StringBuffer content_procurement_dial1 = new StringBuffer();
StringBuffer content_procurement_dial2 = new StringBuffer();
StringBuffer content_procurement_dial3 = new StringBuffer();
StringBuffer content_procurement_dial4 = new StringBuffer();
...
try {
   ...
   if (rs != null) rs.close(); rs = null;
   if (ps != null) ps.close(); ps = null;
   title = "Procurement Metrics for " + product_name;
...
ChartHelper.doChart("breadboard", "supply_chain/procurement/dashboards",
                    "procure_product_vert_bar.widget.xml", parameters,
...
ChartHelper.doChart("breadboard", "supply_chain/procurement/dashboards",
                    "procure_product_bu_vert_bar.widget.xml", parameters,
...
//use the chart definition in 'samples/dashboard/regions.widget.xml'
ChartHelper.doChart("breadboard", "supply_chain/procurement/dashboards",
                    "procure_product_ware_vert_bar.widget.xml", parameters,
...
//call the chart helper to generate the bar chart image and to get the HTML content
ChartHelper.doChart("breadboard", "supply_chain/procurement/dashboards",
                    "procure_product_supplier_vert_bar.widget.xml", parameters,
  ...
  messages = new ArrayList();
   ...
   parameters.setParameter( "title", "Avg Days Requisition to Warehouse" );
   ChartHelper.doDial("breadboard", "supply_chain/procurement/dashboards", "procure_dial.widget.xml",
                      parameters, content_procurement_dial1, userSession, messages, null); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
  ...
  messages = new ArrayList();
   ...
   parameters.setParameter( "title", "Avg Days Requisition to PO" );
   ChartHelper.doDial("breadboard", "supply_chain/procurement/dashboards", "procure_dial.widget.xml",
                      parameters, content_procurement_dial2, userSession, messages, null); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
  ...
  messages = new ArrayList();
   ...
   parameters.setParameter( "title", "Avg Days PO to Ship" );
   ChartHelper.doDial("breadboard", "supply_chain/procurement/dashboards", "procure_dial.widget.xml",
                      parameters, content_procurement_dial4, userSession, messages, null); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
  ...
  messages = new ArrayList();
   ...
   parameters.setParameter( "title", "Avg Days Ship to Warehouse" );
   ChartHelper.doDial("breadboard", "supply_chain/procurement/dashboards", "procure_dial.widget.xml",
                      parameters, content_procurement_dial4, userSession, messages, null); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
...
for (int i =0; i <  prod_ids.size(); i++) {
	String prod_name = (String)prod_names.get(i);
	String prod_id = (String)prod_ids.get(i);
	selObj_product += "<option value=\"" +prod_id+ "\">" +prod_name+ "</option>\n";
}
...
<html>
<head>
<title>Breadboard BI Sample Product Procurement Dashboard</title>
...
<td>
<form name="form1" action="procure_interactive_dashboard.jsp" method="GET">
<select name="product_sk" onChange="submitNewoption_product(this)">
<option value="<%= product_sk %>">Select a Product</option>
<%= selObj_product %>
</select>
...
<tr><td>
<%= content_procurement_dial1.toString() %>
</td></tr>
<tr><td>
<%= content_procurement_dial2.toString() %>
</td></tr>
<tr><td>
<%= content_procurement_dial3.toString() %>
</td></tr>
<tr><td>
<%= content_procurement_dial4.toString() %>
<!-- Bottom of #3 -->
</td></tr>
</table>
...
<td valign="bottom" align="center">
<form name="form2" action="procure_interactive_dashboard.jsp" method="GET">
<select name="business_unit_sk" onChange="submitNewoption_bu(this)">
<option value="<%= business_unit_sk %>">Select a Store</option>
...
<tr>
<td valign="bottom" align="center">
<form name="form3" action="procure_interactive_dashboard.jsp" method="GET">
<select name="warehouse_sk" onChange="submitNewoption_warehouse(this)">
<option value="<%= warehouse_sk %>">Select a Warehouse</option>
<%= selObj_warehouse %>
</select>
<INPUT TYPE=HIDDEN NAME="product_sk" value="<%= product_sk %>">
<INPUT TYPE=HIDDEN NAME="business_unit_sk" value="<%= business_unit_sk %>">
...
<td valign="bottom" align="center">
<form name="form4" action="procure_interactive_dashboard.jsp" method="GET">
