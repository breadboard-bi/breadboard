...
String sql = "";
String sql2 = "SELECT MAX(PURCHASE_ORDER_DATE) AS PURCHASE_ORDER_DATE FROM FACT_PURCHASE_ORDER";
int myResolved = 0;
int mySLAcomp = 0;
...
if (category.length() > 0) {
   title = "Purchase Orders by " + temporal_value + " and " + business_unit + " and " + category;
   report_title = "Top Products by " + temporal_value + " and " + business_unit + " and " + category;
   dial_title = "Period PO Trends for " + business_unit + " and " + category;
} else if ( business_unit.length() > 0)  {
   title = "Purchase Orders by " + temporal_value + " and " + business_unit;
   ...
   title = "Purchase Orders for " + temporal_value;
   report_title = "Purchase Orders for " + temporal_value;
   dial_title = "Period PO Trends for " + temporal_value;
}
...
try {
   ...
   rs = ps.executeQuery();
   if (rs.next()) {
      PURCHASE_ORDER_DATE = "Updated with purchase order data as of "+rs.getString("PURCHASE_ORDER_DATE")+" PST";
   }
   if (rs != null) rs.close(); rs = null;
   if (ps != null) ps.close(); ps = null;
...
if (temporal_cat == "Year of " || temporal_cat == "Quarter of ") {
   ...
   parameters.setParameter( "drill-url", "po_interactive_line_dashboard.jsp?temporal_value={SERIES}" );
}
...
ChartHelper.doChart("breadboard", "supply_chain/purchase_orders/dashboards",
                    "po_period_vert_bar.widget.xml", parameters,
...
//define the click url template
parameters.setParameter("drill-url", "po_interactive_line_dashboard.jsp?temporal_value="
...
ChartHelper.doChart("breadboard", "supply_chain/purchase_orders/dashboards",
                    "po_period_bu_vert_bar.widget.xml", parameters,
...
parameters = new SimpleParameterProvider();
// define the click url template
parameters.setParameter( "drill-url", "po_interactive_line_dashboard.jsp?temporal_value="+temporal_value
...
//use the chart definition in 'samples/dashboard/regions.widget.xml'
ChartHelper.doChart("breadboard", "supply_chain/purchase_orders/dashboards",
                    "po_period_bu_pcat_vert_bar.widget.xml", parameters,
...
SolutionHelper.doAction("breadboard", "supply_chain/purchase_orders/dashboards/reports",
                     "po_top_products_report.xaction", "po_interactive_line_dashboard.jsp",
...
// use the chart definition in 'samples/dashboard/regions.widget.xml'
ChartHelper.doChart( "breadboard", "supply_chain/purchase_orders/dashboards", 
					  "po_period_bu_pcat_line.widget.xml", parameters, 
...
<html>
<head>
<title>Breadboard BI Sample Purchase Order Dashboard</title>
...
<form name="form1" action="po_interactive_line_dashboard.jsp" method="GET">
