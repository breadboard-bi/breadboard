...
String sql = "";
String sql2 = "SELECT MAX(SHIPPING_NOTICE_DATE) AS SHIPPING_NOTICE_DATE FROM FACT_SHIPPING_NOTICE";
int myResolved = 0;
int mySLAcomp = 0;
...
// if (category.length() > 0) {
   title = "Shipping Notices by " + temporal_value + " & " + business_unit + " & " + category;
...
//   title = "Shipping Notices by " + temporal_value + " & " + business_unit;
...
//   title = "Shipping Notices for " + temporal_value;
//   report_title = "Shipping Notices for " + temporal_value;
//   line_chart_title = "Period Trends for " + temporal_value;
// }
String SHIPPING_NOTICE_DATE = "";
...
try {
   ...
   rs = ps.executeQuery();
   if (rs.next()) {
      SHIPPING_NOTICE_DATE = "Updated with shipping notice data as of "+rs.getString("SHIPPING_NOTICE_DATE")+" PST";
   }
   if (rs != null) rs.close(); rs = null;
   if (ps != null) ps.close(); ps = null;
...
if (temporal_cat == "Year of " || temporal_cat == "Quarter of ") {
   ...
   parameters.setParameter( "drill-url", "sn_interactive_line_dashboard.jsp?temporal_value={SERIES}" );
}
...
ChartHelper.doChart("breadboard", "supply_chain/shipping_notices/dashboards",
                    "sn_period_vert_bar.widget.xml", parameters,
...
//define the click url template
parameters.setParameter("drill-url", "sn_interactive_line_dashboard.jsp?temporal_value="
...
ChartHelper.doChart("breadboard", "supply_chain/shipping_notices/dashboards",
                    "sn_period_bu_vert_bar.widget.xml", parameters,
...
parameters = new SimpleParameterProvider();
// define the click url template
parameters.setParameter( "drill-url", "sn_interactive_line_dashboard.jsp?temporal_value="+temporal_value
...
//use the chart definition in 'samples/dashboard/regions.widget.xml'
ChartHelper.doChart("breadboard", "supply_chain/shipping_notices/dashboards",
                    "sn_period_bu_pcat_vert_bar.widget.xml", parameters,
...
//use the chart definition in 'samples/dashboard/regions.widget.xml'
ChartHelper.doChart("breadboard", "supply_chain/shipping_notices/dashboards",
                    "sn_period_bu_pcat_ware_vert_bar.widget.xml", parameters,
...
SolutionHelper.doAction("breadboard", "supply_chain/shipping_notices/dashboards/reports",
                     "sn_top_products_report.xaction", "sn_interactive_line_dashboard.jsp",
...
// use the chart definition in 'samples/dashboard/regions.widget.xml'
ChartHelper.doChart( "breadboard", "supply_chain/shipping_notices/dashboards", 
					  "sn_period_bu_pcat_ware_line.widget.xml", parameters, 
...
<html>
<head>
<title>Breadboard BI Sample Shipping Notices Dashboard</title>
...
<form name="form1" action="sn_interactive_line_dashboard.jsp" method="GET">
