...
String web_file_sk = ((request.getParameter("web_file_sk")!=null)?request.getParameter("web_file_sk"):"");
String web_file_name = ((request.getParameter("web_file_name")!=null)?request.getParameter("web_file_name"):"");
...
String default_web_file_sk = "";
String selObj_file_name = "";
...
String sql_default_file = "SELECT WEB_FILE_SK, WEB_FILE_NAME FROM DIMENSION_WEB_FILE WHERE WEB_FILE_NAME = '/index.html' AND HOSTED_CLIENT_SK = ?";
String sql_file_name = "SELECT WEB_FILE_NAME FROM DIMENSION_WEB_FILE WHERE WEB_FILE_SK = ? AND HOSTED_CLIENT_SK = ?";
String sql = "";
...
String sql_file = "SELECT DISTINCT D.WEB_FILE_SK AS file_id, D.WEB_FILE_NAME AS file_name FROM DIMENSION_WEB_FILE D, FACT_WEB_PAGE_VIEW F WHERE D.HOSTED_CLIENT_SK = ? AND F.WEB_FILE_SK = D.WEB_FILE_SK ORDER BY D.WEB_FILE_NAME";
ArrayList file_ids = new ArrayList();
...
try {
   ...
   //get the next query results, sql_default_file
   ps = conn.prepareStatement(sql_default_file);
   ps.setString(1, hosted_client_sk);
   rs = ps.executeQuery();
   if (rs.next()) {
      default_web_file_sk = ((rs.getString("WEB_FILE_SK")!=null)?rs.getString("WEB_FILE_SK"):"");
   }
   if (rs != null) rs.close(); rs = null;
   if (ps != null) ps.close(); ps = null;

if (web_file_sk.length() < 1)
	web_file_sk = default_web_file_sk;
   ...
   rs = ps.executeQuery();
   if (rs.next()) {
      update_date = "Updated with page view data as of "+((rs.getString("REQUEST_DATE")!=null)?rs.getString("REQUEST_DATE"):"")+" PST";
   }
   if (rs != null) rs.close(); rs = null;
   if (ps != null) ps.close(); ps = null;
   ...
   //now get the next query results, String sql_file
   ps = conn.prepareStatement(sql_file);
   ps.setString(1, hosted_client_sk);
   rs = ps.executeQuery();
   while (rs.next()) {
    String file_id = ((rs.getString("file_id")!=null)?rs.getString("file_id"):"");
    String file_name = ((rs.getString("file_name")!=null)?rs.getString("file_name"):"");
   ...
   //get the next query results, sql_file_name
   ps = conn.prepareStatement(sql_file_name);
   ps.setString(1, web_file_sk);
   ps.setString(2, hosted_client_sk);
   rs = ps.executeQuery();
   if (rs.next()) {
      web_file_name = ((rs.getString("WEB_FILE_NAME")!=null)?rs.getString("WEB_FILE_NAME"):"");
   }
   if (rs != null) rs.close(); rs = null;
   if (ps != null) ps.close(); ps = null;
...
title = "Views for the " + web_file_name + " Page During the " + temporal_cat + " " + temporal_value;
...
// parameters.setParameter( "drill-url", "web_page_interactive_dashboard.jsp?source_system_sk="+source_system_sk+"&amp;temporal_value={SERIES}" );
parameters.setParameter( "SOURCE_SYSTEM_SK", source_system_sk);	      
parameters.setParameter( "WEB_FILE_SK", web_file_sk);	
...
ChartHelper.doChart( "breadboard", "customer_360/web_analytics/dashboards", 
					  "web_page_period_vert_bar.widget.xml", parameters, content_1, 
...
// define the click url template
// parameters.setParameter( "drill-url", "web_page_interactive_dashboard.jsp?source_system_sk="+source_system_sk+"&amp;temporal_value="+temporal_value+"&amp;country={SERIES}" );
// define the bars of the bar chart
parameters.setParameter( "TEMPORAL_VALUE", temporal_value);
parameters.setParameter( "WEB_FILE_SK", web_file_sk);
parameters.setParameter( "HOSTED_CLIENT_SK", hosted_client_sk);
...
// call the chart helper to generate the bar chart image and to get the HTML content
ChartHelper.doChart( "breadboard", "customer_360/web_analytics/dashboards", 
					 "web_page_period_seconds_vert_bar.widget.xml", parameters, content_2, 
					 userSession, messages, null ); 
...
parameters = new SimpleParameterProvider();
// define the click url template
// parameters.setParameter("drill-url","web_visit_interactive_dashboard_3.jsp?source_system_sk="+source_system_sk+"&amp;temporal_value="+temporal_value+"&amp;country="+country+"&amp;region={SERIES}" );
parameters.setParameter( "SOURCE_SYSTEM_SK", source_system_sk);
parameters.setParameter( "TEMPORAL_VALUE", temporal_value);
parameters.setParameter( "HOSTED_CLIENT_SK", hosted_client_sk);
parameters.setParameter( "WEB_FILE_SK", web_file_sk);
...
// call the chart helper to generate the bar chart image and to get the HTML content
ChartHelper.doChart( "breadboard", "customer_360/web_analytics/dashboards", 
					 "web_page_period_landing_vert_bar.widget.xml", parameters, 
...
// call the chart helper to generate the bar chart image and to get the HTML content
ChartHelper.doChart( "breadboard", "customer_360/web_analytics/dashboards", 
					 "web_page_period_exit_vert_bar.widget.xml", parameters, 
...
// call the chart helper to generate the bar chart image and to get the HTML content
ChartHelper.doChart( "breadboard", "customer_360/web_analytics/dashboards", 
					 "web_page_period_exit_ratio_vert_bar.widget.xml", parameters, 
					 content_5, userSession, messages, null ); 
...
// call the chart helper to generate the bar chart image and to get the HTML content
ChartHelper.doChart( "breadboard", "customer_360/web_analytics/dashboards", 
					 "web_page_period_stickiness_vert_bar.widget.xml", parameters, 
					 content_6, userSession, messages, null ); 
...
for (int i =0; i <  file_ids.size(); i++) {
	...
	String file_id = (String)file_ids.get(i);
	selObj_file_name += "<option value=\"" +file_id+ "\">" +file_name+ "</option>\n";
}
...
<html>
<head>
<title>Breadboard BI Sample Web Page Analysis Dashboard</title>
...
<form name="form1" action="web_page_interactive_dashboard.jsp" method="GET">
...
<select name="web_file_sk">
<option value="<%= web_file_sk %>">Select a Web Page</option>
<%= selObj_file_name %>
</select>
