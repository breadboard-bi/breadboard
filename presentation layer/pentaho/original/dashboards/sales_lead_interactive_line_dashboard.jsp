...
String sql = "";
String sql2 = "SELECT MAX(CREATED_DATE) AS CREATED_DATE FROM FACT_SALES_LEAD";
int myResolved = 0;
int mySLAcomp = 0;
...
if (category.length() > 0) {
   title = "Sales Leads by " + temporal_value + " and " + country + " and " + category;
   report_title = "Top Products by " + temporal_value + " and " + country + " and " + category;
   dial_title = "Period Sales Lead Trends for " + country + " and " + category;
} else if ( country.length() > 0)  {
   title = "Sales Leads by " + temporal_value + " and " + country;
   report_title = "Top Products by " + temporal_value + " and " + country;
   dial_title = "Period Sales Lead Trends for " + temporal_value + " and " + country;
} else if (temporal_value.length() > 0)  {
   title = "Sales Leads for " + temporal_value;
   report_title = "Sales Leads for " + temporal_value;
   dial_title = "Period Sales Lead Trends for " + temporal_value;
}
String created_date = "";
...
StringBuffer content_sales_lead_trend_line = new StringBuffer();
...
try {
   ...
   rs = ps.executeQuery();
   if (rs.next()) {
      created_date = "Updated with sales lead data as of "+rs.getString("CREATED_DATE")+" PST";
   }
   if (rs != null) rs.close(); rs = null;
   if (ps != null) ps.close(); ps = null;
...
if (temporal_cat == "Year of " || temporal_cat == "Quarter of ") {
   ...
   parameters.setParameter( "drill-url", "sales_lead_interactive_line_dashboard.jsp?temporal_value={SERIES}" );
}
...
ChartHelper.doChart("breadboard", "customer_360/leads/dashboards",
                    "sales_lead_period_vert_bar.widget.xml", parameters,
...
//define the click url template
parameters.setParameter("drill-url", "sales_lead_interactive_line_dashboard.jsp?temporal_value="
...
//call the chart helper to generate the bar chart image and to get the HTML content
ChartHelper.doChart("breadboard", "customer_360/leads/dashboards",
                    "sales_lead_period_country_vert_bar.widget.xml", parameters,
...
parameters = new SimpleParameterProvider();
// define the click url template
parameters.setParameter( "drill-url", "sales_lead_interactive_line_dashboard.jsp?temporal_value="+temporal_value
...
//call the chart helper to generate the bar chart image and to get the HTML content
ChartHelper.doChart("breadboard", "customer_360/leads/dashboards",
                    "sales_lead_period_country_pcat_vert_bar.widget.xml", parameters,
...
messages = new ArrayList();
SolutionHelper.doAction("breadboard", "customer_360/leads/dashboards/reports",
                     "sales_lead_top_products_report.xaction", "sales_lead_interactive_line_dashboard.jsp",
...
ChartHelper.doChart( "breadboard", "customer_360/leads/dashboards", 
					  "sales_lead_period_country_pcat_line.widget.xml", parameters, 
					  content_sales_lead_trend_line, userSession, messages, null ); 
...
<html>
<head>
<title>Breadboard BI Sample Sales Sales Lead Dashboard</title>
...
<form name="form1" action="sales_lead_interactive_line_dashboard.jsp" method="GET">
...
<img src='/pentaho/images/show-calendar.gif' width=24 height=22 bsales_lead=0 alt=''>
</a>
<input type='button' name='dateBtn' value="Go" onClick='doDateChange()'>
Note:  test data available for 11/2005 - 06/2006.
</form>
...
<!-- Top of 1 -->
                <span style="font-family:Arial;font-weight:bold"> Select a Lead Creation Period</span>
...
<!-- Top #2 -->
                <span style="font-family:Arial;font-weight:bold">Select a Country</span>
...
<!--			Click on an Sales Lead ID to drill to another page (not activated) -->
<BR>
...
<!-- Top of #6 -->
<%= content_sales_lead_trend_line.toString() %>
